...
public class CheckEmptyContainerRelease : MonoBehaviour
{
    ...
    private static float PenaltyScore = 2;
    ...
    void Update(){
        ...
        for (int i = ObjectList.Count - 1; i >= 0; i--)
        {
            ...
            // }
            if (CheckAnimalHabitat.ReleaseSuccessCount + CheckAnimalHabitat.ReleaseErrorCount == ObjectList.Count){
                Counter++;
            }
    ...
    public void AddScore(){
        TimeScore = TimeBonusScore*CheckAnimalHabitat.ReleaseSuccessCount*RemainingTime/TotalTime*FullPercent;
        AreaScore+=(int)System.Math.Floor(TimeScore);
        ErrorScore = CheckAnimalHabitat.ReleaseErrorCount*PenaltyScore;
        if(AreaScore-ErrorScore<0){
            AreaScore=0;
        }else{
            AreaScore-=(int)System.Math.Floor(ErrorScore);
        }
